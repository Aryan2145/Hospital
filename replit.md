...
- **Episode Model Freeze (Level 1):** Simplified episode data model answering 5 CRM questions: (1) Case Owner → `doctorId`, (2) Surgery Doctor → `surgeryDoctorId` (nullable), (3) Post-Care Owner → `postCareOwnerId` (nullable, references crmUsers), (4) Quote → `initialQuote` / `approvedDiscount` / `finalQuote` (auto-calculated), (5) Billing → `actualBill` / `variance` (auto-calculated). Episode Detail Page rewritten with 3-tab layout: Clinical (details + case ownership + clinical notes), Financial (quote & billing + discount request + revenue), Insurance (toggle + insurer/TPA/preauth). Family tab removed. Old columns remain in DB for backward compat. Discount approval/revoke endpoints sync both new simplified fields and legacy fields.
