...
- **Automated Nurture Engine (Layer 2):** Background scheduler (`server/services/backgroundScheduler.ts`) runs every 30 minutes across all active tenants. Processes: (1) Dormant leads (5+ days no activity) → creates re-engage tasks + marks temperature as Dormant, (2) Stale appointments (Appointment Booked with past date, no consultation) → creates high-priority follow-up tasks, (3) Overdue nurture tasks (48+ hours) → escalates to manager (deduplicated). No-show automation: 2+ no-shows auto-moves lead to Nurture and starts the nurture task chain.
